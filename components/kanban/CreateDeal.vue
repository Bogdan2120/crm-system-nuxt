...
<template>
    <div class=" text-center mb-2">
        <button class=" transition-all opacity-5 hover:opacity-100 hover:text-[#a252c8]" @click="isOpenForm = !isOpenForm">
            <Icon v-if="isOpenForm" name="radix-icons:arrow-up" class="fade-in-100 fade-in-0" size="35" />
            <Icon v-else name="radix-icons:plus-circled" class="fade-in-100 fade-in-0" size="35" />
        </button>

        <form v-if="isOpenForm" @submit="onSubmit" class="form">
            <UiInput placeholder="Deal name" v-model="name" v-bind="nameAttrs" type="text" class="input" />
            <UiInput placeholder="Price" v-model="price" v-bind="priceAttrs" type="text" class="input" />
            <UiInput placeholder="Company name" v-model="customerName" v-bind="customerNameAttrs" type="text"
                class="input" />
